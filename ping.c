...
int limit_detection=0;//Whether limit detection is enabled.



int pkg_num=10;//-c send how many package in the program
...

{
    ...
    while ( (c = getopt(argc, argv, "t:vfc:")) != -1) {

        switch (c) {
	    case 't':
		ttl=atoi(optarg);
		printf("The time to live is set as %d \n",ttl);
		break;
		...
		break;
	    case 'c':
		pkg_num = optarg[0] - '0';//argument is tranformed from String to int
		printf("the program will send %d package\n",pkg_num);
		break;
...

{
    ...
    //set ttl according to argument -t in IP level
    if(ttl!=128)
	{
  	  printf("ttl in readloop:%d \n",ttl);
	}	
    ...
    for (int i = 0; i < pkg_num; i++) {
        ...
        (*pr->fsend)();
	if(!limit_detection)//when limit_detection is enabled,then wait and print
	{
        	sleep(1);
